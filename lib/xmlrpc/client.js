...
Client.prototype._methodCall =
    async function _methodCall(method, params, cb, retryCount) {
      ...
      const callback = async function(err, value) {
        if (err && retryCount <= 5) {
          if (err != 'Error: XML-RPC fault: Wrong username or password.') {
            if (err != 'Error: XML-RPC fault: Account is temporarily blocked') {
      ...
      const request = https.request(options, function(response) {
        ...
        if (response.statusCode == 404) {
          ...
          callback(__enrichError(
              new Error('There was a problem calling "' +
                method + '(' + params.join(', ') + ')' +
                `" on the specified path "${options.path}": ` +
                response.statusCode),
          ));
        }
      }.bind(this));
