...
BoaClient.prototype._get_output = async function(job) {
  return new Promise((resolve, reject) =>
    this.client.methodCall('job.output', [job.id],
        function(error, value) {
          ...
          let warning = '\n\n';
          warning += 'NOTE: Output is too long, ';
          warning += 'only the first 64k is shown...\n';
          ...
          require('https').get(value, {
            ...
            res.on('data', function(chunk) {
              ...
              if (buff.length >= maxSize) {
                res.destroy();
                buff = Buffer.concat([buff.slice(0, maxSize),
                  Buffer.from(warning)]);
...
BoaClient.prototype._get_output_full = async function(job) {
  return new Promise((resolve, reject) =>
    this.client.methodCall('job.output', [job.id],
        function(error, value) {
          if (error) {
