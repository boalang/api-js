...
const truncateWarning = '\n\n' +
  'NOTE: Output too big, only the first 64k is shown...\n';

BoaClient.prototype._get_output = async function(job, maxOutputSize=undefined) {
  return new Promise((resolve, reject) =>
    this.client.methodCall('job.output', [job.id],
        function(error, value) {
          ...
          require('https').get(value, function(res) {
            res.on('data', function(chunk) {
              buff = Buffer.concat([buff, chunk]);
              if (maxOutputSize && buff.length >= maxOutputSize) {
                res.destroy();
                buff = Buffer.concat([buff.slice(0, maxOutputSize),
                  Buffer.from(truncateWarning)]);
