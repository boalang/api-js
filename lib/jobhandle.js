...
class JobHandle {
  ...
  get output() {
    ...
    return this.#client._get_output(this, 0, this.#outputSize);
  }
  ...
  /**
   * Downloads the job's output to a file.
   * @param {string} filename the name of the file to download output to
   * @param {number} [start=0] the starting byte of the output
   * @param {number} [length] the (max) length of the output, defaults to all
   * @return {boolean} if the download was successful
   */
  async downloadOutput(filename, start=0, length=undefined) {
    if (this.#executionStatus != ExecutionStatus.FINISHED) {
      throw Error('Job is currently running');
    }
    return this.#client._download_output(this, filename, start, length);
  }
